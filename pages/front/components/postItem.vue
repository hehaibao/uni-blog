<template>
	<div class="post-item" @click="toDetail">
		...
		<el-image v-if="props.item.thumbnail" class="post-item-thumbnail" :src="props.item.thumbnail" fit="contain"></el-image>
...
const toDetail = () => {
	if (!props.item.password) {
		router.navigateTo(`front/detail?id=${props.item._id}`)
	} else {
		// 加密文章
