<template>
	<div class="container">
		<div class="tools">
			...
			<el-button @click="edit()" type="primary">写文章</el-button>
		...
		<el-table :data="tableData" v-loading="loading" @selection-change="handleSelectionChange">
			...
			<el-table-column label="标题">
				<template #default="scope">
					<el-link :href="`/#/pages/front/detail?id=${scope.row._id}`" target="_blank">{{ scope.row.title }}</el-link>
				</template>
			</el-table-column>
			...
			<el-table-column label="缩略图" :width="100">
				<template #default="scope">
					<el-image v-if="scope.row.thumbnail" :src="scope.row.thumbnail" :preview-src-list="[scope.row.thumbnail]" fit="contain" style="width: 50px; height: 50px" />
				</template>
			</el-table-column>
...
import { SyncAlt, Search } from '@vicons/fa'
...
// 编辑
const edit = id => {
	router.redirectTo(id ? `admin/writing?id=${id}` : 'admin/writing')
}
