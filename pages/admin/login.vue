<template>
	<div class="login-wrap">
		<el-card>
			<template #header>
				<span>{{ needRegister ? '注册并自动登录' : '系统登录' }}</span>
			</template>
			<el-input v-model="form.username" placeholder="账号"></el-input>
			<el-input v-model="form.password" type="password" placeholder="密码"></el-input>
			<el-button type="primary" :loading="loading" :disabled="disabled" @click="submit">{{ needRegister ? '注册' : '登录' }}</el-button>
		</el-card>
	</div>
</template>
...
let from = ''
onLoad(option => {
	from = option.from // 从某个页面跳过来的，登录成功后跳回去
})
...
<style lang="scss">
body{
	background-image: linear-gradient(to right, #53a8ff, #C6EAC1);
}
.login-wrap {
	.el-card {
		width: 320px;
