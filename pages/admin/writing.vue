	...
	<div class="container writing-wrap">
		...
		<el-drawer v-model="showSetting" title="文章设置" :size="400" direction="rtl">
			<el-form label-position="top">
				...
				<el-form-item label="摘要"><el-input v-model="form.abstract" type="textarea" placeholder="不填写则截取内容前100个字符" /></el-form-item>
				<el-form-item label="项目类型">
					<el-radio-group v-model="form.type">
					  <el-radio :label="1" size="large">文章</el-radio>
					  <el-radio :label="2" size="large">项目</el-radio>
					</el-radio-group>
				</el-form-item>
				<el-form-item label="所属分类" v-if="form.type==1"><get-categorys v-model:categorys="form.categorys" /></el-form-item>
				<el-form-item label="标签" v-if="form.type==1"><get-labels v-model:labels="form.labels" /></el-form-item>
				...
				<el-form-item label="封面图">
					...
					<div v-else class="thumbnail-box">
						<el-image @click="openAttachment(true)" fit="contain" :src="form.thumbnail" style="width: 280px;height: 150px;" />
...
const submit = (status, msg) => {
	...
	if (status === 1 && !form.value.abstract) {
		...
		let str = form.value.html
			...
			.replace(/ /g, ' ')
			.replace(/>/g, ' ')
		form.value.abstract = str.substring(0, 100)
	}
	// 自动生产标题
