...
// 计算日期
const date = dayjs(new Date()).diff(dayjs('2014-08-18'), 'day')
...
call('getAggregatePosts').then(res => {
	statistics.value[0].value = res.data.total || 0 // 文章总数
	statistics.value[1].value = res.data.totalPageView || 0 // 总浏览量
})
</script>
