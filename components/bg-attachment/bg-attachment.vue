<template>
	...
	<el-drawer v-model="show" title="附件库" :size="380" direction="rtl" destroy-on-close>
		...
		<div v-loading="form.loading">
			<el-empty v-if="attachments.length === 0" :description="form.loading ? '查找中...' : '什么也没找到'" :image-size="120"></el-empty>
			<div v-else class="imags-box"><el-image v-for="item in attachments" @click="select(item)" :src="item.url" style="width: 165px; height: 120px;" fit="contain" /></div>
		</div>
		<el-divider />
		<!-- 分页 -->
