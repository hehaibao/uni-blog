<template>
	<div class="bg-editor" :class="{ expand: expand, 'only-edit': onlyShow === 'edit', 'only-html': onlyShow === 'html' }">
		...
		<div class="editor-tools">
			<!-- 撤回 -->
			<div @click="undo" class="tool-box" title="撤回">
				<Icon :size="16"><UndoAlt /></Icon>
			</div>
			<!-- 复原 -->
			<div @click="redo" class="tool-box divider-r" title="复原">
				<Icon :size="16"><RedoAlt /></Icon>
			</div>
			<!-- 样式按钮 -->
			<div @click="setStyle(tool)" v-for="tool in tools" class="tool-box" :title="tool.title">
			...
			<!-- 全屏 -->
			<div @click="expand = !expand" class="tool-box divider-l" title="全屏">
				<Icon :size="16"><ExpandArrowsAlt /></Icon>
			</div>
			<!-- 只显示编辑区 -->
			<div @click="changeShow('edit')" class="tool-box" title="只显示编辑区">
				<Icon :size="16"><WindowMaximizeRegular /></Icon>
			</div>
			<!-- 只显示 html 区 -->
			<div @click="changeShow('html')" class="tool-box" title="只显示预览区">
